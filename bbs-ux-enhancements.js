...
(() => {
  ...
  const syncFamily = (button) => {
    ...
    if (control && [...control.options].some(option => option.value === family) && control.value !== family) {
      control.value = family;
  ...
  document.addEventListener('change', event => {
    if (event.target.matches('[data-bbs-field="uBaseLength"], [data-bbs-field="uLeg1"], [data-bbs-field="uLeg2"]')) setTimeout(decorate, 0);
  }, true);
