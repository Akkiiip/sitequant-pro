...
window.SiteQuant.bbs = (() => {
  ...
  function summary(rows) {
    const projectAnalysis = model.analysis(projectId);
    ...
    find('#bbs-summaries').innerHTML = `<section><h3>Total steel</h3><strong>${view.format(totals.kg, 2)} kg</strong><small>${totals.bars} bars · ${view.format(totals.length)} m · ${view.format(totals.tonnes)} t</small></section><details><summary>Member-wise</summary><ul>${grouped('member')}</ul></details><details><summary>Diameter-wise</summary><ul>${grouped('dia')}</ul></details><details><summary>Shape-wise</summary><ul>${grouped('shape')}</ul></details><details><summary>Family-wise</summary><ul>${family || '<li>No calculated items.</li>'}</ul></details><section class="bbs-procurement"><h3>Wastage estimate · Professional</h3><p>Net theoretical: ${view.format(projectAnalysis.total.weight,2)} kg<br>Allowance: ${projectAnalysis.wastage.percent}% / ${view.format(projectAnalysis.wastage.allowanceKg,2)} kg<br><b>Estimated procurement: ${view.format(projectAnalysis.wastage.procurementKg,2)} kg</b></p><small>The theoretical BBS quantity is unchanged. Stock optimization — Coming next.</small></section><details><summary>Cutting list · Professional</summary><ul>${projectAnalysis.cutting.map(x=>`<li><b>Ø${x.diameter} ${x.grade} · ${x.shape} · ${x.cutLength} m</b><span>${x.family} · ${x.quantity} bars · ${view.format(x.totalLength)} m · ${view.format(x.totalWeight,2)} kg</span></li>`).join('') || '<li>No calculated items.</li>'}</ul></details>`;
  }
