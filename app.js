document.addEventListener('DOMContentLoaded', () => {
  ...
  function simple(title, desc) {
    return `<section>${header('WORKSPACE', title, desc)}<div class="empty panel"><b>This workspace is being developed.</b><span>Use Overview for project drafts and sample activity, or BBS for the current calculation workflow.</span><p><a class="sq-text-link" href="#/overview">Back to overview →</a></p></div></section>`;
  }
  ...
  function showError(error) {
    ...
    view.innerHTML = `<section class="sq-surface sq-empty-state"><h1>Workspace unavailable</h1><p>${html(message)}</p><button class="sq-button sq-button--secondary" data-reload>Reload workspace</button><p class="sq-notice">If this persists after a reload, the browser is holding an incompatible cached asset set.</p></section>`;
  ...
  function navigation() {
    ...
    document.addEventListener('click', event => {
      ...
      if (routeButton?.dataset.exportType) { const m=window.SiteQuant.bbsModel, rows=m.exportData(routeButton.dataset.exportProject,routeButton.dataset.exportType); window.SiteQuant.ui.downloadCsv(rows,`SiteQuant_${routeButton.dataset.exportProject}_${routeButton.dataset.exportType}.csv`); m.recordExport(routeButton.dataset.exportProject,routeButton.dataset.exportType,Math.max(0,rows.length-1)); notify('CSV generated from current browser-local BBS data.'); }
    });
