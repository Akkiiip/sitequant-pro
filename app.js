document.addEventListener('DOMContentLoaded', () => {
  ...
  function exportsPage(){const rows=[['BBS CSV','Calculated BBS rows','Available now','Open BBS → Export view'],['BBS summary CSV','Member, diameter, shape and family totals','Available now','Open BBS → summaries'],['Diameter summary','Calculated diameter totals','Available now','Open BBS → summaries'],['Member summary','Calculated member totals','Available now','Open BBS → summaries'],['PDF report','Construction review report','Coming next','No file generated']];return `<section>${header('EXPORT WORKSPACE','Exports','Exports operate on browser-local calculated BBS data only.')}<div class="panel"><table><tr><th>Export type</th><th>Project</th><th>Records</th><th>Generated</th><th>Status</th><th>Action</th></tr>${rows.map(r=>`<tr><td>${r[0]}</td><td>Selected BBS project</td><td>${r[1]}</td><td>On download</td><td>${r[2]}</td><td>${r[3].startsWith('Open')?'<a class="sq-text-link" href="#/bbs">'+r[3]+'</a>':r[3]}</td></tr>`).join('')}</table><p class="sq-notice">No export history, cloud files or PDF generation is implied. CSV download is the implemented export.</p></div></section>`;}
