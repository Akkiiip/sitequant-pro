document.addEventListener('DOMContentLoaded', () => {
  ...
  const engineVersion = () => {
    try {
      return window.SiteQuant.bbsModel?.calculate?.(window.SiteQuant.bbsModel.defaults)?.result?.engineVersion || 'Versioned engine';
    } catch {
      return 'Versioned engine';
    }
  };
  ...
  const bbs = () => {
    if (!window.SiteQuant.bbs || typeof window.SiteQuant.bbs.render !== 'function') {
      throw new Error('BBS workspace module did not load. Refresh once to load the current workspace assets.');
    }
    return window.SiteQuant.bbs.render();
  };
  ...
  function wire() {
    if (state.page === 'bbs') {
      const workspace = view.querySelector('.bbs-workspace');
      if (!workspace) throw new Error('BBS rendered without its workspace root.');
      if (!window.SiteQuant.bbs || typeof window.SiteQuant.bbs.mount !== 'function') throw new Error('BBS controller did not load.');
      window.SiteQuant.bbs.mount(workspace);
      return;
    }
    view.querySelectorAll('table').forEach(table => {
      if (table.closest('.sq-table-scroll')) return;
      const region = document.createElement('div'); region.className = 'sq-table-scroll'; region.tabIndex = 0; region.setAttribute('role', 'region'); region.setAttribute('aria-label', 'Quantity table'); table.before(region); region.append(table);
    });
  }
  function showError(error) {const message = error instanceof Error ? error.message : String(error);view.innerHTML = `<section class="sq-surface sq-empty-state"><h1>Workspace unavailable</h1><p>${html(message)}</p><button class="sq-button sq-button--secondary" data-reload>Reload workspace</button><p class="sq-notice">If this persists after a reload, the browser is holding an incompatible cached asset set.</p></section>`;view.querySelector('[data-reload]').onclick = () => location.reload();console.error('SiteQuant boot error', error);}
  function render(page = 'overview', focus = true) {
    try {
      window.SiteQuant.bbs?.unmount?.();
      state.page = Object.hasOwn(nav, page) ? page : 'overview';
      crumb.textContent = nav[state.page]; document.title = nav[state.page] + ' · SiteQuant Pro';
      ...
      document.querySelectorAll('[data-nav-page]').forEach(link => { if (link.dataset.navPage === state.page) link.setAttribute('aria-current', 'page'); else link.removeAttribute('aria-current'); });
      wire();
      if (focus) { view.focus({ preventScroll: true }); window.scrollTo(0, 0); }
  ...
  function navigation() {
    ...
    document.addEventListener('click', event => {
      ...
      if (exportButton) { const type=exportButton.dataset.exportType,projectId=exportButton.dataset.exportProject||'riverside',m=window.SiteQuant.bbsModel,rows=m.exportData(projectId,type); window.SiteQuant.ui.downloadCsv(rows,`SiteQuant_${projectId}_${type}.csv`); m.recordExport(projectId,type,Math.max(0,rows.length-1)); notify('CSV generated from current browser-local BBS data.'); return; }
      const routeButton = event.target.closest('[data-page]'); if (routeButton) location.hash = '#/' + routeButton.dataset.page;
      const link = event.target.closest('a[href^="#/"]'); if (link) document.querySelectorAll('dialog[open]').forEach(dialog => dialog.close());
    });
    window.addEventListener('hashchange', () => { if (location.hash === '#view') return; render(location.hash.replace(/^#\//, '') || 'overview'); });
  }
  navigation();
  if (!window.SiteQuant.dashboard || !window.SiteQuant.bbsModel) { showError(new Error('Core workspace assets did not initialize.')); return; }
  window.SiteQuant.dashboard.init(() => { if (state.page !== 'overview') location.hash = '#/overview'; else render('overview', false); });
