 ...
 * This is intentionally independent of the product UX bootstrap. It runs after the SPA router
 * and patches the rendered BBS DOM directly, so route timing cannot suppress the visible UX.
 */
(() => {
  let lastRoot = null;
  let busy = false;
  ...
  function removeReviewWorkflow(root) {
    root.querySelectorAll('.bbs-fieldset').forEach(fs => {
      ...
      if (legend === '02 review workflow') fs.remove();
    });
    ...
    if (statusIndex >= 0) {
      ...
      root.querySelectorAll('.bbs-table tbody tr').forEach(row => row.children[statusIndex]?.remove());
    }
  }
  ...
  function normalizeDirection(root) {
    ...
    const parent = select.closest('.bbs-field');
    const label = parent?.querySelector('label');
    if (label) label.firstChild.textContent = 'Bar arrangement';
    const hint = root.querySelector('#bbs-direction-hint');
    if (hint) hint.textContent = select.value === 'Distribution'
      ? 'Distribution bars run across the member and are spaced along the main span.'
      : 'Main bars follow the member span and are distributed across the perpendicular dimension.';
  }

  function collapseResult(root) {
    const result = root.querySelector('.bbs-result');
    if (!result || result.dataset.directCollapsed === '1') return;
    const heading = result.querySelector('.bbs-result-heading');
    if (!heading) return;
    result.dataset.directCollapsed = '1';
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'bbs-result-toggle bbs-direct-toggle';
    button.setAttribute('aria-expanded', 'false');
    button.innerHTML = '<span>Details</span><span aria-hidden="true">›</span>';
    heading.appendChild(button);
    const details = document.createElement('div');
    details.className = 'bbs-result-details bbs-direct-details';
    [...result.children].filter(n => n !== heading).forEach(n => details.appendChild(n));
    result.appendChild(details);
    result.classList.add('bbs-result--collapsed');
    button.addEventListener('click', () => {
      const open = !result.classList.contains('bbs-result--expanded');
      result.classList.toggle('bbs-result--expanded', open);
      result.classList.toggle('bbs-result--collapsed', !open);
      button.setAttribute('aria-expanded', String(open));
      button.innerHTML = `<span>${open ? 'Hide' : 'Details'}</span><span aria-hidden="true">${open ? '‹' : '›'}</span>`;
    });
  }

  function shapeCardDiagrams(root) {
    ['C','I','E','F'].forEach(code => {
      root.querySelectorAll(`.bbs-shape[data-bbs-shape="${code"] .bbs-diagram`).forEach(old => {
        if (!old.classList.contains('bbs-direct-diagram')) old.outerHTML = svg(code);
      });
    });
  ...
  function linkCopy(root) {
    root.querySelectorAll('*').forEach(el => {
      if (el.children.length === 0 && el.textContent.includes('max(6d, 65 mm)')) el.textContent = el.textContent.replaceAll('max(6d, 65 mm)', 'max(10d, 75 mm)');
      if (el.children.length === 0 && el.textContent.includes('6d,65')) el.textContent = el.textContent.replaceAll('6d,65', '10d,75');
    });
  }
  ...
  function apply() {
    ...
    if (!root || root === lastRoot && root.dataset.directReady === '1') return;
    busy = true;
    try {
      removeReviewWorkflow(root);
      normalizeDirection(root);
      shapeCardDiagrams(root);
      collapseResult(root);
      linkCopy(root);
      root.dataset.directReady = '1';
      lastRoot = root;
    } finally {
      busy = false;
    }
  ...
  const observer = new MutationObserver(() => {
    const root = document.querySelector('.bbs-workspace');
    if (!root) return;
    if (root !== lastRoot || root.dataset.directReady !== '1') setTimeout(apply, 0);
  });
